...
### Page Reads, Prefetching, and Loading

Page-rendering reads use feature-owned Pinia Colada Query factories. The Discussions list can start the detail Query before navigation completes, while the shared Delay plugin postpones the background loading indicator for short refetches. See the [Pinia Colada Practices](../practices/pinia-colada/index.md) for reusable guidance on Query organization, prefetching, and loading presentation.
