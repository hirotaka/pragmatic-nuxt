...
const { createError, findById, requireUserSession } = vi.hoisted(() => ({
  createError: vi.fn((input: { statusCode: number; message: string }) => Object.assign(new Error(input.message), input)),
  findById: vi.fn(),
...
describe("requireCurrentUser", () => {
  ...
  it("loads the current database user from the session identity", async () => {
    ...
    requireUserSession.mockResolvedValue({ user: { id: "user-1", role: "ADMIN" } });
