...
async function registerFetchHook() {
  const { default: registerSessionPlugin } = await import("../session");
...
test("projects the current allowlisted user onto the fetch response", async () => {
  ...
  const session = { id: "provider-session", user: { id: user.id, role: "USER" } };
...
test.each([
  { id: "provider-session" },
  { id: "provider-session", user: null },
])("leaves an anonymous session unauthenticated", async (session) => {
  const fetchHook = await registerFetchHook();
...
test.each([
  ...
  { id: 42 },
  "",
])("rejects a malformed present identity before querying the repository", async (user) => {
  const fetchHook = await registerFetchHook();

  await expect(fetchHook({ id: "provider-session", user }, {})).rejects.toMatchObject({
    statusCode: 401,
