...
const { addNotification, canAccessAdmin, clearSession, mockRoute, mockUser, reloadNuxtApp, routerPush } = vi.hoisted(() => ({
...
test("dashboard layout logs out before reloading the public root", async () => {
  ...
  await waitFor(() => expect(reloadNuxtApp).toHaveBeenCalledWith({ path: "/" }));
  expect(clearSession.mock.invocationCallOrder[0]).toBeLessThan(
    reloadNuxtApp.mock.invocationCallOrder[0]!,
  );
});
...
test("dashboard layout ignores duplicate logout events while the action is pending", async () => {
  ...
  logoutDone.resolve();
  await waitFor(() => expect(reloadNuxtApp).toHaveBeenCalledOnce());
});
