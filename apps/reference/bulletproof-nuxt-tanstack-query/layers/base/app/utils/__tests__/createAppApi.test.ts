...
test.runIf(import.meta.client)("configures a browser transport without incoming cookies", () => {
...
test("keeps the caller AbortSignal when issuing an API request", () => {
  const api = vi.fn();
  createMock.mockReturnValue(api);
  const controller = new AbortController();

  const client = createAppApi();
  client("/api/comments", { signal: controller.signal });
