...
const { mockUser, addNotification, invalidateQueries, refreshSession, session } = vi.hoisted(() => ({
  mockUser: {
    value: {
      id: "user-1",
...
vi.mock("@tanstack/vue-query", async importOriginal => ({
  ...
  useMutation: (options: { mutationFn?: (input: unknown, context: never) => Promise<unknown> }) => {
    const isPending = ref(false);
    return {
      isPending,
      mutateAsync: async (input: unknown) => {
        if (!options.mutationFn) throw new Error("Profile mutation function is unavailable");
        isPending.value = true;
        try {
          return await options.mutationFn(input, {} as never);
        }
        finally {
