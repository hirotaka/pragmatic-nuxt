...
test("direct users collection is SSR-rendered without a hydration GET", { tag: ["@users", "@ssr"] }, async ({ page }) => {
  ...
  expect(users).toMatchObject({
    ...
    meta: { page: 1, limit: 10, total: expect.any(Number), totalPages: expect.any(Number) },
  });
...
test("user delete settles before its detached users refresh", { tag: ["@users", "@mutation-refresh"] }, async ({ page, goto }) => {
