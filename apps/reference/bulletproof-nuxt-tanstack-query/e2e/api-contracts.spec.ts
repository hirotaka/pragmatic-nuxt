...
async function expectPaginatedUsers(request: APIRequestContext, path = "/api/users") {
  const result: unknown = await expectJson(await request.get(path));
  if (!isApiRecord(result) || !Array.isArray(result.data) || !isApiRecord(result.meta)) {
    throw new Error("Users response does not match the paginated contract");
  }
...
async function expectUserPresent(request: APIRequestContext, userId: string) {
  const users = await expectPaginatedUsers(request, "/api/users?limit=100");
  expect(users.data.some(user => isApiRecord(user) && user.id === userId)).toBe(true);
}
...
test("collection pagination enforces the route contract", { tag: ["@contract", "@discussions", "@comments", "@users"] }, async ({ page }) => {
...
test("user deletion is confined to the admin team", { tag: ["@contract", "@users"] }, async ({ page }) => {
  ...
  try {
    ...
    const remaining = await expectPaginatedUsers(page.request, "/api/users?limit=100");
    expect(remaining.data.some(user => isApiRecord(user) && user.id === sameTeam.user.id)).toBe(false);
    expect(remaining.data.some(user => isApiRecord(user) && user.id === crossTeam.user.id)).toBe(false);
  }
  finally {
    await Promise.all([
...
test("datetime fields are actual JSON ISO strings across auth, teams, users, discussions, and comments", { tag: ["@contract", "@cross-domain"] }, async ({ page }) => {
  ...
  const users = await expectPaginatedUsers(page.request);
  expectIsoString((users.data[0] as Record<string, unknown>).createdAt);
