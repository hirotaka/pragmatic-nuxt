...
test("smoke", async ({ page, goto }) => {
  ...
  const deleteCommentDialog = page.getByRole("dialog", { name: "Delete Comment" });
  await expect(
    deleteCommentDialog.getByText("Are you sure you want to delete this comment?"),
  ).toBeVisible();
  ...
  await page.getByRole("dialog", { name: "Delete Discussion" }).waitFor({ state: "visible" });
  await page.keyboard.press("Tab");
  await page.keyboard.press("Enter");
  await page
