...
const users = computed(() => unref(data)?.data ?? []);

const adminCount = computed(() => users.value.filter(user => user.role === "ADMIN").length);
const memberCount = computed(() => users.value.filter(user => user.role !== "ADMIN").length);
