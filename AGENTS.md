...
- Canonical Nuxt app (`apps/bulletproof-nuxt`) verified local lifecycle and structural checks:
  ...
  - `pnpm test:e2e:ci` (fresh local migration/build/preview/Playwright lifecycle; hosted CI remains unverified)
- Do not run remote reset or deploy commands in this subitem. Hosted CI and Cloudflare D1 lifecycle evidence remain deferred to B-003-04 and B-003-05.
...
- Nuxt CI (`.github/workflows/nuxt-ci.yml`) still runs, per Nuxt app: `build` -> `test:unit` -> `lint` -> `nuxi typecheck`, plus separate `test:e2e:ci` job. Local lifecycle evidence is complete for B-003-03; hosted CI evidence remains B-003-04.
